<template>
  <v-stepper v-model="step">
    <v-stepper-header>
      <v-stepper-step :complete="step > 1" step="1">Info. General</v-stepper-step>
      <v-divider/>
      <v-stepper-step :complete="step > 2" step="2">Descripción</v-stepper-step>
      <v-divider/>
      <v-stepper-step :complete="step > 3" step="3">Especificaciones</v-stepper-step>
      <v-divider/>
      <v-stepper-step step="4">Caracteres de SKU</v-stepper-step>
    </v-stepper-header>
    <v-stepper-items>
      <!--1、基本信息-->
      <v-stepper-content step="1">
        <v-flex class="xs10 mx-auto">
          <v-form v-model="valid" ref="basic">
            <v-layout row>
              ...
              <v-flex xs5>
                ...
                <v-select
                  ...
                  clearable
                  dense chips
                  :n="[v => !!v || 'La marca no puede ser nulo.']"
                >
            ...
            <v-text-field label="Título de productos" v-model="goods.title" :counter="200" required :rules="[v => !!v || 'El título no puede ser nulo.']" hide-details/>
            <v-text-field label="Subtítulo" v-model="goods.subTitle" :counter="200" hide-details/>
            <v-text-field label="Embalaje" v-model="goods.spuDetail.packingList" :counter="1000" multi-line :rows="3" hide-details/>
            <v-text-field label="Post Venta" v-model="goods.spuDetail.afterService" :counter="1000" multi-line :rows="3" hide-details/>
          </v-form>
        </v-flex>
      </v-stepper-content>
      <!--2、商品描述-->
      ...
      <v-stepper-content step="4">
          ...
          <v-card flat v-for="spec in specialSpecs" :key="spec.name">
            ...
            <v-card-text class="px-5">
              <div v-for="i in spec.options.length+1" :key="i" class="layout row px-5">
                <v-text-field :placeholder="'Nuev@ ' + spec.name + ':'" class="flex xs10" auto-grow
        ...
        <v-flex xs3 offset-xs9>
          <v-btn color="info" @click="submit">Guardar</v-btn>
        </v-flex>
      </v-stepper-content>
    </v-stepper-items>
  </v-stepper>
  ...
  methods: {
    submit() {
      ...
      if(!this.$refs.basic.validate){
        this.$message.error("¡Por favor, que termine de cumplimentar el formulario!");
      }
